...
class DIBX_Module_Core extends ET_Builder_Module {
     ...
     * @return module_icon
     */
    public function dibx_module_icon($icon_name) {
        
        return sprintf(
            '%1$s/assets/icons/%2$s.svg',
            DIBX_DIVI_BUILDER_X_DIR,
            $icon_name
        );
    }
     ...
     * @return void
     */
    public function dibx_responsive_css($render_slug, $style_options = array()) {
        ...
        foreach ($style_options as $style) {
            ...
            if (class_exists('ET_Builder_Element')) {
                if (isset($desktop) && !empty($desktop)) {
                    ET_Builder_Element::set_style($render_slug, array(
                        'selector' => $options['selector'],
                        'declaration' => $this->dibx_conditional_css($property, $desktop, $important),
                    ));
                }
                ...
                if (isset($phone) && !empty($phone)) {
                    ET_Builder_Element::set_style($render_slug, array(
                        'selector' => $options['selector'],
                        'declaration' => $this->dibx_conditional_css($property, $phone, $important),
                ...
                if (et_builder_is_hover_enabled($option_slug, $module->props) && isset($module->props[$option_slug . '__hover'])) {
                    ...
                    ET_Builder_Element::set_style($render_slug, array(
                        ...
                        'declaration' => $this->dibx_conditional_css($property, $hover, $important),
                    ));
                }
            }
        }
    ...
    public function dibx_conditional_css( $property, $value, $important ) {
    ...
    protected function dibx_custom_background_fields(
        $base_name,
        $label,
        $tab_slug,
        $toggle_slug,
        ...
        $help_text       = esc_html__('Adjust background style of this element by customizing the background options', 'dibx-divi-nations');
    ...
    protected function dibx_process_bg_styles($option_name, $hover_suffix) {
    ...
    protected function dibx_custom_bg_style($render_slug, $option_slug, $selector, $hover_selector) {

        $background = $this->dibx_process_bg_styles($option_slug, '');
        ...
        $hover_background = $this->dibx_process_bg_styles($option_slug, '__hover');
    ...
    public function render_prev_icon() {
        ...
        dibx_inject_fa_icons($this->props['prev_icon']);
     ...
     * Iocn Option Name
     */
    public function dibx_render_icon($option_name) {
        
        // Inject Font Awesome Manually!.
        dibx_inject_fa_icons($this->props[$option_name]);
    ...
    public function render_next_icon() {
        ...
        // Inject Font Awesome Manually!.
        dibx_inject_fa_icons($this->props['next_icon']);
